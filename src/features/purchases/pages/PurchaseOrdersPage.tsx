...
export function PurchaseOrdersPage() {
  ...
  const columns: Column<Document>[] = [
    { key: 'document_number', header: 'شماره سفارش خرید' },
    { key: 'party_display_name', header: 'تأمین‌کننده / فروشنده' },
    {
      key: 'grand_total',
      header: 'ارزش سفارش تدارکاتی',
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="سفارشات خرید (درخواست تأمین)"
        description="ثبت درخواست‌های خرید، پیش‌فاکتورهای دریافتی از همکاران و پیگیری تأمین کالا"
        icon={<FileSignature className="w-6 h-6 text-violet-600" />}
        actions={
          <div className="flex items-center gap-2">
            <Link to="/purchases/new?type=purchase_order">
              <Button
                variant="primary"
                size="sm"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-5 border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 rounded-2xl flex items-center justify-between shadow-sm">
          <div>
            <p className="text-xs text-slate-400 font-semibold">ارزش کل سفارشات خرید</p>
        ...
        <Card className="p-5 border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 rounded-2xl flex items-center justify-between shadow-sm">
          <div>
            <p className="text-xs text-slate-400 font-semibold">سفارشات معلق و باز</p>
            <p className="text-lg font-black text-amber-600 mt-1">{stats.activeCount} سفارش تاییدشده</p>
          </div>
        ...
        <Card className="p-5 border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 rounded-2xl flex items-center justify-between shadow-sm">
          <div>
            <p className="text-xs text-slate-400 font-semibold">تأمین و دریافت‌شده</p>
            <p className="text-lg font-black text-emerald-600 mt-1">{stats.convertedCount} سفارش</p>
          </div>
        ...
        <Card className="p-5 border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 rounded-2xl flex items-center justify-between shadow-sm">
          <div>
            ...
            <p className="text-lg font-black text-slate-600 mt-1">{stats.draftCount} پیش‌نویس</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/20 text-slate-600">
            <Clock className="w-5 h-5" />
          </div>
        </Card>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[200px] gap-2">
          ...
          <p className="text-xs text-slate-500">در حال دریافت سفارشات خرید...</p>
        </div>
      ) : (
        <DataTable
          columns={columns}
          ...
          searchPlaceholder="جستجو بر اساس نام تأمین‌کننده یا شماره سفارش..."
